...
The API is available from the host at `http://localhost:8080` because Docker Compose maps `8080:8080`. Inside the Compose network, the API connects to PostgreSQL with `DB_HOST=db` instead of `localhost`.

...
Tapping a conversation in `ConversationsScreen` pushes `ConversationScreen` with `Navigator.push` (a `go_router` migration is a separate, later phase). Each conversation screen owns a `ConversationCubit` that loads history over REST (`GET /conversations/{id}/messages/{userId}`) and subscribes to `ChatRepository.watchConversation(id)` for live updates, merging by message `id` so the REST snapshot and the realtime echo of a just-sent message never duplicate.

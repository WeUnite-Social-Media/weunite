...
var useIsMobile_1 = require("@/shared/hooks/useIsMobile");
...
var tooltip_1 = require("@/shared/components/ui/tooltip");
...
function Sidebar(_a) {
    ...
    return (<div className="group peer text-sidebar-foreground hidden md:block" data-state={state} data-collapsible={state === "collapsed" ? collapsible : ""} data-variant={variant} data-side={side} data-slot="sidebar">
      ...
      <div data-slot="sidebar-container" className={(0, utils_1.cn)("fixed inset-y-0 z-30 hidden h-svh w-(--sidebar-width) transition-[left,right,width] duration-200  ease-linear md:flex", side === "left"
...
function SidebarTrigger(_a) {
    ...
    return (<button_1.Button data-sidebar="trigger" data-slot="sidebar-trigger" variant="ghost" size="icon" className={(0, utils_1.cn)("size-7 hover:cursor-pointer", className)} onClick={function (event) {
...
function SidebarMenuSub(_a) {
    var className = _a.className, props = __rest(_a, ["className"]);
    return (<ul data-slot="sidebar-menu-sub" data-sidebar="menu-sub" className={(0, utils_1.cn)("border-sidebar-border mx-3.5 flex min-w-0 translate-x-px flex-col gap-1 border-l px-2.5 py-0.5", "group-data-[collapsible=icon]:hidden [&>svg]:size-4 [&>svg]:shrink-0", className)} {...props}/>);
}

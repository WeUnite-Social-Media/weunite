...
function CreatePost(_a) {
    ...
    var createPostMutation = (0, usePosts_1.useCreatePost)();
                ...
                switch (_a.label) {
                    case 0:
                        if (!(user === null || user === void 0 ? void 0 : user.id))
                            return [2 /*return*/];
                        return [4 /*yield*/, createPostMutation.mutateAsync({
                                data: values,
    ...
    var isSubmitting = createPostMutation.isPending;
    return (<dialog_1.Dialog open={open} onOpenChange={onOpenChange}>
      <dialog_1.DialogContent className="sm:max-w-[500px]" aria-describedby={undefined}>
        <dialog_1.DialogHeader>
          <dialog_1.DialogTitle>Criar nova publicação</dialog_1.DialogTitle>
        </dialog_1.DialogHeader>
        <form onSubmit={form.handleSubmit(onSubmit)} className="grid gap-4">
          <div className="grid gap-4 py-2">
            <div className="grid gap-2">
              <label_1.Label htmlFor="post-text">
                O que você gostaria de compartilhar?
              </label_1.Label>
              <react_hook_form_1.Controller name="text" control={form.control} render={function (_a) {
            var field = _a.field;
            return (<textarea_1.Textarea id="post-text" placeholder="Escreva sua mensagem aqui..." className="min-h-[150px]" {...field}/>);
        }}/>
            </div>
            <div className="grid gap-2">
              <label_1.Label htmlFor="post-image">Adicionar imagem (opcional)</label_1.Label>
              <react_hook_form_1.Controller name="media" control={form.control} render={function (_a) {
            var _b = _a.field, onChange = _b.onChange, name = _b.name;
            return (<input_1.Input id="post-image" name={name} type="file" accept="image/*, video/*" onChange={function (e) {
                    var _a;
          ...
          <dialog_1.DialogFooter className="mt-4">
            ...
            <button_1.Button type="submit" className="variant-third bg-third hover:bg-third-hover" disabled={isSubmitting} aria-busy={isSubmitting}>

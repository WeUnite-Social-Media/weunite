...
var createOpportunity_schema_1 = require("@/features/opportunities/schemas/opportunity/createOpportunity.schema");
...
function CreateOpportunity(_a) {
    var open = _a.open, onOpenChange = _a.onOpenChange;
    var form = (0, react_hook_form_1.useForm)({
        resolver: (0, zod_1.zodResolver)(createOpportunity_schema_1.createOpportunitySchema),
        defaultValues: {
            title: "",
            description: "",
            location: "",
    ...
    var createOpportunityMutation = (0, useOpportunities_1.useCreateOpportunity)();
                ...
                switch (_a.label) {
                    case 0:
                        if (!(user === null || user === void 0 ? void 0 : user.id))
                            return [2 /*return*/];
                        return [4 /*yield*/, createOpportunityMutation.mutateAsync({
                                data: __assign(__assign({}, values), { skills: values.skills.map(function (skillName, index) { return ({
                                        id: index + 1,
                                        name: skillName,
                                    }); }) }),
    ...
    var isSubmitting = createOpportunityMutation.isPending;
    return (<dialog_1.Dialog open={open} onOpenChange={onOpenChange}>
      <dialog_1.DialogContent className="w-[90vw] max-w-[40em] max-h-[90vh] overflow-y-auto p-4" aria-describedby={undefined}>
        <dialog_1.DialogHeader className="pb-2">
          <dialog_1.DialogTitle className="text-lg">Criar Oportunidade</dialog_1.DialogTitle>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="grid gap-2 text-sm">
          ...
          <div className="grid gap-1">
            ...
            return (<dropdown_menu_1.DropdownMenu>
                  <dropdown_menu_1.DropdownMenuTrigger asChild>
                    <button_1.Button variant="outline" className={(0, utils_1.cn)("w-full justify-between text-left font-normal h-8 text-sm", !field.value && "text-muted-foreground")}>
                      <div className="flex items-center">
                        ...
                        {field.value ? ((0, date_fns_1.format)(field.value, "dd/MM/yyyy", { locale: locale_1.ptBR })) : (<span>Selecionar data</span>)}
                      </div>
                      <lucide_react_1.ChevronDownIcon className="h-3 w-3 opacity-50"/>
                    </button_1.Button>
                ...
                }} fixedWeeks={true} initialFocus locale={locale_1.ptBR}/>
          ...
          <div className="grid gap-2">
            ...
            return (<>
                  ...
                  {field.value.length > 0 && (<SelectedSkills_1.SelectedSkills skills={field.value} showRemove={true} onRemoveSkill={function (skill) {

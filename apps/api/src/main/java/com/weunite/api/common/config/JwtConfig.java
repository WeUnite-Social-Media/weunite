...
public class JwtConfig {
  ...
  private RSAPublicKey loadPublicKey() throws Exception {
    String publicKeyPem =
        new String(Base64.getDecoder().decode(jwtPublicKeyBase64Env), StandardCharsets.UTF_8);
  ...
  private RSAPrivateKey loadPrivateKey() throws Exception {
    String privateKeyPem =
        new String(Base64.getDecoder().decode(jwtPrivateKeyBase64Env), StandardCharsets.UTF_8);
  ...
  public JwtEncoder jwtEncoder(RSAPublicKey publicKey, RSAPrivateKey privateKey) {
    RSAKey rsaKey =
        new RSAKey.Builder(publicKey).privateKey(privateKey).algorithm(JWSAlgorithm.RS256).build();

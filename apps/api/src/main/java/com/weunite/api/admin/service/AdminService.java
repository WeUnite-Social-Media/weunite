package com.weunite.api.admin.service;
...
public class AdminService {
  ...
  public ResponseDTO<String> dismissReports(Long entityId, String type) {
    ...
    return new ResponseDTO<>(
        "Denúncias descartadas com sucesso", reports.size() + " denúncias foram descartadas");
  }
  ...
  private void markReportsAsReviewed(Long entityId, Report.ReportType type) {
    List<Report> reports =
        reportRepository.findByEntityIdAndTypeAndStatus(
            entityId, type, Report.ReportStatus.PENDING);
    reports.forEach(report -> report.setStatus(Report.ReportStatus.REVIEWED));
    reportRepository.saveAll(reports);
  }
    ...
    return results.stream()
        .map(
            result -> {
              ...
              return new ReportedPostDetailDTO(postDTO, reportDTOs, reportCount, "pending");
            })
        .collect(Collectors.toList());
  }
        ...
        .map(
            result -> {
              ...
              return new ReportedOpportunityDetailDTO(
                  opportunityDTO, reportDTOs, reportCount, "pending");
            })
        .collect(Collectors.toList());
  }

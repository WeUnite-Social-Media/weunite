...
class AuthServiceTest {
  ...
  void loginSuccess() {
    ...
    AuthDTO authData = new AuthDTO(expectedUserDTO, "jwt-token", 3600L);

    ResponseDTO<AuthDTO> expectedResponse =
        new ResponseDTO<>("Login realizado com sucesso!", authData);
  ...
  void loginWithUnverifiedEmail() {
    ...
    NotVerifiedEmailException exception =
  ...
  void signUpSuccess() {
    ...
    ResponseDTO<AuthDTO> expectedResponse =
        new ResponseDTO<>("Cadastro concluído! Verifique seu email", null);
  ...
  void verifyEmailSuccess() {
    ...
    String roleName =
        mockUser.getRole() != null && !mockUser.getRole().isEmpty()
            ? mockUser.getRole().iterator().next().getName()
            : "BASIC";
    UserDTO expectedUserDTO =
        new UserDTO(
            mockUser.getId().toString(),
    ...
    ResponseDTO<AuthDTO> expectedResponse =
        new ResponseDTO<>("Email verificado com sucesso!", authData);
  ...
  void verifyEmailWithUserNotFound() {
    ...
    when(userService.findUserEntityByEmail(email)).thenThrow(new UserNotFoundException());
  ...
  @Test
  @DisplayName(
      "Should send reset password email successfully when user exists and email is verified")
  void sendResetPasswordSuccess() {
    ...
    ResponseDTO<AuthDTO> expectedResponse = new ResponseDTO<>("Código enviado!", null);
  ...
  void sendResetPasswordWithNonExistentUser() {
    SendResetPasswordRequestDTO requestDTO =
        new SendResetPasswordRequestDTO("nonexistent@example.com");

    when(userService.findUserEntityByEmail("nonexistent@example.com"))
        .thenThrow(new UserNotFoundException());

    UserNotFoundException exception =
        assertThrows(UserNotFoundException.class, () -> authService.sendResetPassword(requestDTO));
  ...
  void sendResetPasswordWithUnverifiedEmail() {
    SendResetPasswordRequestDTO requestDTO =
        new SendResetPasswordRequestDTO("unverified@example.com");
    ...
    NotVerifiedEmailException exception =
        assertThrows(
            NotVerifiedEmailException.class, () -> authService.sendResetPassword(requestDTO));
  ...
  void verifyResetPasswordTokenSuccess() {
    ...
    ResponseDTO<AuthDTO> expectedResponse = new ResponseDTO<>("Código verificado!", null);
  ...
  void resetPasswordSuccess() {
    ...
    ResponseDTO<AuthDTO> expectedResponse = new ResponseDTO<>("Senha redefinida!", null);
  ...
  void resetPasswordWithInvalidToken() {
    ...
    UserNotFoundException exception =
        assertThrows(
            UserNotFoundException.class, () -> authService.resetPassword(requestDTO, invalidToken));
